...
<head>
    ...
    <style>
        ...
        /* BRACKET CARDS */
        .battle-box { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 15px; display: flex; flex-direction: column; gap: 10px; transition: 0.3s; position: relative; }
        .battle-box-small { padding: 10px; opacity: 0.7; }
        .battle-box:hover { border-color: var(--accent-pink); transform: scale(1.02); background: rgba(255,255,255,0.04); }
        .battle-n { font-family: 'Inter Tight', sans-serif; font-size: 0.6rem; font-weight: 900; color: var(--accent-pink); text-transform: uppercase; letter-spacing: 0.1em; }
        .battle-qf-label { position: absolute; top: 15px; right: 15px; font-size: 0.5rem; color: var(--accent-cyan); font-weight: 900; opacity: 0.5; }
        
        .battle-team { display: flex; align-items: center; gap: 10px; position: relative; }
        .team-info { display: flex; flex-direction: column; gap: 1px; }
        .team-song { font-size: 0.75rem; font-weight: 900; color: #fff; line-height: 1.1; }
        .team-artist { font-size: 0.6rem; color: #888; }
        .team-score { margin-left: auto; font-family: 'Inter Tight', sans-serif; font-weight: 900; font-size: 0.9rem; color: var(--accent-cyan); }
        
        /* PROX BAR */
        .score-bar-bg { width: 100%; height: 3px; background: rgba(255,255,255,0.05); border-radius: 2px; overflow: hidden; margin-top: 4px; }
        .score-bar-fill { height: 100%; background: var(--accent-pink); transition: 1s width ease-in-out; }
        
        .team-win { border-left: 3px solid var(--accent-acid); padding-left: 8px; background: rgba(204, 255, 0, 0.02); }
        .team-lose { opacity: 0.3; filter: grayscale(1); }
        .winner-tag { color: var(--accent-acid); font-size: 0.6rem; margin-left: 5px; }
        
        @keyframes live-pulse { 0% { border-color: var(--accent-pink); box-shadow: 0 0 5px var(--accent-pink); } 50% { border-color: #fff; box-shadow: 0 0 20px var(--accent-pink); } 100% { border-color: var(--accent-pink); box-shadow: 0 0 5px var(--accent-pink); } }
        .status-live { animation: live-pulse 2s infinite; border-width: 2px; background: rgba(255,0,127,0.05); }

        .country-bubble { background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); border-radius: 10px; padding: 10px; display: flex; flex-direction: column; align-items: center; gap: 6px; transition: 0.3s; }
        .country-bubble:hover { background: rgba(255,255,255,0.08); border-color: var(--accent-cyan); }
    ...
    <script>
        ...
        function render() {
            ...
            document.getElementById('bracket-body').innerHTML = DATA.battles.map(b => {
                const isFinished = b.status === 'FINISHED' || b.status === 'LIVE';
                return `
                <div class="battle-box ${b.status === 'LIVE' ? 'status-live' : ''} ${!isFinished ? 'battle-box-small' : ''}">
                    ...
                    <div class="battle-team ${b.w===1?'team-win':(b.w===2?'team-lose':'')}">
                        ...
                        <span class="team-score">${b.sc1}${b.w===1?' <span class="winner-tag">★</span>':''}</span>
                    </div>
                    
                    ${isFinished ? `<div class="score-bar-bg"><div class="score-bar-fill" style="width:${b.sc1}"></div></div>` : ''}
                    
                    <div class="vs-label" style="margin: 5px 0;">VS</div>
                    ...
                    <div class="battle-team ${b.w===2?'team-win':(b.w===1?'team-lose':'')}">
                        ...
                        <span class="team-score">${b.sc2}${b.w===2?' <span class="winner-tag">★</span>':''}</span>
                    </div>
                    
                    ${isFinished && b.sc2 ? `<div class="score-bar-bg"><div class="score-bar-fill" style="width:${b.sc2}; background:var(--accent-cyan);"></div></div>` : ''}
                </div>
            `}).join('');
...
with open('index.html', 'w', encoding='utf-8') as f:
    f.write(final_html)
print("Interactive Hub with fixed logic overwritten successfully.")
