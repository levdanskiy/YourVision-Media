...
from lexicon_common import DATABASES, REPORTS, iter_posts, parse_date_target, split_values
...
def render(target: str) -> str:
    ...
    month_posts = iter_posts(month, prefixes={"LX", "SV", "SP"})
    previous = [post for post in month_posts if post.publication_date < day and post.prefix == "LX"]
    ...
    lines = [
        ...
        "",
        "## Rotation",
        f"- Prepared LX before date: {len(previous)}; recent 14 days: {len(recent)}.",
        ...
        f"- Almanac handoff: standalone oracle/calendar now live as LEXICON slot material, not service debt. ORACLE-NOTE={weekly_oracle}; CALENDAR-RADAR={weekly_radar}.",
