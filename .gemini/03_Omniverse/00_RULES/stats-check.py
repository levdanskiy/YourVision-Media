...
# Код работы -> папка проекта относительно TREE_ROOT
WORKS = {
    # 01_NOVELLS
    "eleyia": "01_NOVELLS/01_ELEYIA",
    "kingmaker": "01_NOVELLS/02_KINGMAKER",
    "orchid": "01_NOVELLS/03_ORCHID",
    "white_lady": "01_NOVELLS/04_WHITE_LADY",
    # 03_LITERATURE
    "arcana": "03_LITERATURE/01_ARCANA",
    "zodiac": "03_LITERATURE/02_ZODIAC",
    # 02_STORIES
    "tower": "02_STORIES/01_TOWER",
    "cinnamon_case": "02_STORIES/02_CINNAMON_CASE",
    "parallax": "02_STORIES/03_PARALLAX",
    # 06_CHRONICLES
    "kochevye": "06_CHRONICLES/01_KOCHEVYE",
    "venskaya_krov": "06_CHRONICLES/02_VENSKAYA_KROV",
    "order_of_signs": "06_CHRONICLES/03_ORDER_OF_SIGNS",
    "tyulpan_i_pepel": "06_CHRONICLES/04_TYULPAN_I_PEPEL",
    "panteon": "06_CHRONICLES/05_PANTEON",
}
...
def check_counter(path, node, errors, warns, infos):
    ...
    for i, h in enumerate(history):
        ...
        total_delta += d
        current += d
        # Применяем промежуточные ограничения по ходу истории
        if "max" in node and current > node["max"]:
            current = node["max"]
            has_cap_overflow = True
        if "min" in node and current < node["min"]:
            current = node["min"]
        cap = node.get("cap")
    ...
    if round(value, 2) != expected:
        calc_str = "последовательный расчет с лимитами" if has_cap_overflow else f"start({start})+Σδ({total_delta:+g})"
        errors.append(
            f"{path}: value={value}, а ожидается ({calc_str})={expected} - РАСХОЖДЕНИЕ АРИФМЕТИКИ"
        )
    else:
        calc_str = "последовательный расчет с лимитами" if has_cap_overflow else f"start({start})+Σδ({total_delta:+g})"
        infos.append(f"{path}: value={value} ({calc_str}) ✓")
