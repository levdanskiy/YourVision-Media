...
# Source: .gemini/03_Omniverse/01_NOVELLS/000_GENERAL/tools/pre-commit-novells.sh
# Install: ln -sf "$(pwd)/.gemini/03_Omniverse/01_NOVELLS/000_GENERAL/tools/pre-commit-novells.sh" .git/hooks/pre-commit
#
# Checks performed on staged .md files under 01_NOVELLS:
...
#
# Safe to install repo-wide: only checks files under 01_NOVELLS, ignores everything else.

set -uo pipefail

# Get staged .md files under 01_NOVELLS
STAGED=$(git diff --cached --name-only --diff-filter=ACMR | grep -E "01_NOVELLS.*\.md$" || true)
