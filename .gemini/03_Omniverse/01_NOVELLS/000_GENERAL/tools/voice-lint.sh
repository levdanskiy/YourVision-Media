...
if [ "$MODE" = "--auto" ]; then
  ...
  case "$FILE" in
    ...
    *04_WHITE_LADY/*) PROJECT="white_lady" ;;
...
# Check: эм-тире / эн-тире (универсальное правило - ТОЛЬКО дефис, проверяем весь файл вкл. подвалы/баннеры)
dash_found=$(grep -nE "—|–" "$FILE" 2>/dev/null || true)
if [ -n "$dash_found" ]; then
  echo "❌ Эм/эн-тире найдены (универсально ЗАПРЕЩЕНО - только дефис '-'):"
  echo "$dash_found" | head -10 | sed 's/^/    /'
  ERRORS=$((ERRORS + 1))
fi
