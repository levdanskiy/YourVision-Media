...
template_file = "Apsaimniekošana/2026/1_Sistema_Excel/Invoice_System_2026.xlsx"
...
# 1. READ DATA (from the calculated copy)
print("Reading data...")
if not os.path.exists(source_file):
    # Retrieve calc file if missing
    subprocess.run([
        "libreoffice", "--headless", "--convert-to", "xlsx", 
        template_file, "--outdir", "Apsaimniekošana/2026/1_Sistema_Excel/"
    ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    shutil.move("Apsaimniekošana/2026/1_Sistema_Excel/Invoice_System_2026.xlsx", source_file)
...
for apt in apartments:
    print(f"Generating for Apt {apt['no']}...")
    
    # Load Fresh Template for every iteration to ensure clean state
    wb_templ = openpyxl.load_workbook(template_file)
    
    # REMOVE EXTRA SHEETS
    if 'LISTS' in wb_templ.sheetnames: wb_templ.remove(wb_templ['LISTS'])
    if 'DATI' in wb_templ.sheetnames: wb_templ.remove(wb_templ['DATI'])
    
    ws_inv = wb_templ['RĒĶINS']
    
    # --- Fill Data ---
    ws_inv['B1'] = apt['no']
    ws_inv['B3'] = f"Nr. {inv_date_fmt_iso}-{int(apt['no']):02d}"
    ws_inv['D3'] = inv_date_str
    ws_inv['D4'] = inv_due_str
    ws_inv['D5'] = inv_period
    
    ws_inv['B10'] = apt['name']
    ws_inv['B11'] = f"Parka iela 24, dz. {apt['no']}, Eleja, Jelgavas novads, LV-3023"
    ...
    safe_name = str(apt['name']).replace(" ", "_").replace(".", "")
    final_name = f"Rekins_Dz_{int(apt['no']):02d}_{safe_name}.pdf"
